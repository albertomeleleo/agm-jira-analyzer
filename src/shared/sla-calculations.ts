...
export function isBugOrRequest(issueType: string): boolean {
  const lower = issueType.toLowerCase()
  return BUG_AND_REQUEST_TYPES.some((t) => lower.includes(t))
}

export function isTask(issueType: string): boolean {
  const lower = issueType.toLowerCase()
...
export function calculateSlaMetrics(issues: SLAIssue[]): SlaMetrics {
