...
import { Button, H2, SLACharts, IssueImportModal } from '@design-system'
...
export function SLADashboard(): JSX.Element {
  ...
  const loadData = useCallback(async () => {
    ...
    const m = await window.api.getSlaMetrics(activeProject.name)
    if (m) setMetrics(m)
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {issueCount !== null && (
            <p className="text-sm text-brand-text-sec mt-1">{issueCount} issues imported</p>
          )}
          {lastSyncTime && (
