# agms Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-02-08

## Active Technologies
- TypeScrip + React, Electron, Vite, TailwindCSS (001-sla-dashboard-metrics)
- N/A (Data is read from Jira, not stored locally for this feature) (001-sla-dashboard-metrics)
- TypeScript 5.x + React 18, Electron, Vite, TailwindCSS, Recharts, Lucide-React, Date-fns (007-sla-dashboard-filters)
- LocalStorage (Filter persistence) & Local File System (Jira data cache) (007-sla-dashboard-filters)
- TypeScript 5.3 + React 18, Electron 28, date-fns 4.1, Lucide Reac (008-sla-remaining-column)
- File system (Jira issues and SLA cache in project-specific JSON files) (008-sla-remaining-column)
- TypeScript 5.3 + React 18, Electron 28, TailwindCSS 3.4 (009-issue-status-tag)
- N/A (UI-only change using existing `SLAIssue` data) (009-issue-status-tag)
- N/A (Static mappings in code) (011-jira-localization-mapping)

- TypeScript (as per `tsconfig.json`) + React, Electron, Vite, TailwindCSS (006-dashboard-auto-refresh)

## Project Structure

```text
src/
tests/
```

## Commands

npm test && npm run lint

## Code Style

TypeScript (as per `tsconfig.json`): Follow standard conventions

## Recent Changes
- 011-jira-localization-mapping: Added TypeScript 5.3 + React 18, Electron 28, TailwindCSS 3.4
- 010-issue-key-sorting: Added TypeScript 5.3 + React 18, Electron 28, TailwindCSS 3.4
- 009-issue-status-tag: Added TypeScript 5.3 + React 18, Electron 28, TailwindCSS 3.4


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
